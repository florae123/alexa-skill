 ...
 */
 ...
 function getWeatherCompanyForecast(geocode) {
   ...
   const blocking = true;
   const params = { units: 'm', language: 'de', latitude: geocode[0].toString(), longitude: geocode[1].toString() };
       ...
       case 'lookupWeather':
         ...
         return lookupGeocode(args, watsonResponse.output.location)
           ...
           .then(forecast => {
             ...
             console.log('watsonresponse output after narrative push '+watsonResponse.output.text);
             var temp_text = watsonResponse.output.text[1];
             watsonResponse.output.text[1] = watsonResponse.output.text[2];
             watsonResponse.output.text[2] = temp_text;
             console.log('watsonresponse output after switching '+watsonResponse.output.text);
             resolve(watsonResponse);
           });
       /*case 'set-hauttyp':
          return */
